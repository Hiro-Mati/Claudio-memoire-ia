...
description: Load relevant OpenViking experience memories from configured recall candidates before solving a task.
---
...
3. Review the returned candidates. Each candidate includes a `case_name` or recall-source label. Each experience entry contains its `uri` plus either full inline `content` or a short `situation` snippet describing applicability and exclusions.
4. Each non-empty `content` is already loaded inline; use it directly and do not call `read_experience` again for that entry. Treat `situation` as a filter only, not as the full experience.
5. **Read by default.** For each experience entry without inline `content`, call `read_experience` unless its `situation` explicitly excludes the current task family, object type, policy branch, or requested operation. Do not skip reading merely because the experience may apply later rather than at the opening request.
...
8. After each search, read every candidate without inline `content` that is not explicitly excluded. If multiple experiences pass this gate, read each applicable experience.
