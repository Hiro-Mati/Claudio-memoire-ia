...
def _first_commit_task(tasks: Any, session_id: str) -> dict[str, Any] | None:
    ...
    for task in tasks:
        if not isinstance(task, dict):
            continue
        if task.get("resource_id") != session_id or task.get("task_type") != "session_commit":
            continue
        if task.get("task_id"):
            return task
    return None
...
def _task_archive_uri(task: dict[str, Any] | None) -> str | None:
    if not isinstance(task, dict) or not isinstance(task.get("result"), dict):
        return None
    archive_uri = task["result"].get("archive_uri")
...
def _session_id_for_rollout(
    ...
    *,
    ...
    epoch = context_execution_metadata.get("epoch", embedded_execution_metadata.get("epoch", 0))
